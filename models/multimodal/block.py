...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, attention_module=None):
        ...
        if attention_module is not None:
            ...
            if m_name == "wa" and stride != 1 and in_channels == 16: # 'stride != 1' means the origin conv aims to downsampling
                self.flag = True
    ...
    def __init__(self, in_channels, out_channels, stride=1, attention_module=None):
        ...
        if attention_module is not None:
            ...
            if stride != 1 and m_name == "wa":
                self.flag = True
                self.wa = attention_module()
                self.conv2 = conv3x3(out_channels, out_channels, stride=1)  # conv2 used to be conv3x3(in_channels, out_channels, stride=1)
            else:
