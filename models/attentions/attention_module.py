...
class attention_module(nn.Module):
    ...
    def forward(self, features1, features2, features3, key_padding_mask=None):
        ...
        if key_padding_mask is None:
            output, attn_weights = self.attention(query, key, value)  # when text is key/value, key_padding_mask
        else:
            output, attn_weights = self.attention(query, key, value, key_padding_mask=key_padding_mask)
...
if __name__ == '__main__':
    ...
    key_mask = torch.zeros(2, 6)
    MIXA = attention_module(128, 128)
    output = MIXA(features1, features2, features3, key_padding_mask=key_mask)
    print('Job Done')